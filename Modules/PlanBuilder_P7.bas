...
Private Sub WritePlanRow(ByVal wsPlan As Worksheet, _
    ...
    wsPlan.Cells(r, 2).value = "WP" & Format(CLng(r - PLAN_FIRST_ROW + 1), "00")
    ' Col C = Action
    ...
    End If
    ' Col H is unused in Session E layout (was WP # in P6, collapsed into col B).
    ' Argument `wpNum` retained on the WritePlanRow signature for caller
    ' compatibility but no longer written to a separate column.
