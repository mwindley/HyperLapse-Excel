...
'   AI=Ry,      AJ=Pitch, AK=Δyaw, AL=Δpitch,    AM=Label
' ============================================================
...
Private Sub WriteRowLegacy(ByVal wsLog As Worksheet, ByVal logR As Long, _
    ...
    wsPlan.Cells(planR, 30).value = logR                       ' AD — Log row#
    wsPlan.Cells(planR, 31).value = wsLog.Cells(logR, 1).value ' AE — Time
    ' AF Type left blank
    ' AG AstroTgt left blank
    ' AH KF left blank
    wsPlan.Cells(planR, 35).value = wsLog.Cells(logR, 2).value ' AI — Ry (from Yaw)
    wsPlan.Cells(planR, 36).value = wsLog.Cells(logR, 3).value ' AJ — Pitch
    ' AK Δyaw left blank
    ' AL Δpitch left blank
    wsPlan.Cells(planR, 39).value = wsLog.Cells(logR, 4).value ' AM — Label (from Notes)
End Sub
...
'                         AH=KF, AI=Ry, AJ=Pitch, AK=Δyaw, AL=Δpitch, AM=Label
'
...
Private Sub WriteRowRich(ByVal wsLog As Worksheet, ByVal logR As Long, _
                          ByVal wsPlan As Worksheet, ByVal planR As Long)
    wsPlan.Cells(planR, 30).value = logR                       ' AD LogRow#
    wsPlan.Cells(planR, 31).value = wsLog.Cells(logR, 1).value ' AE Time
    wsPlan.Cells(planR, 32).value = wsLog.Cells(logR, 2).value ' AF Type
    ' AG AstroTgt — leave blank for now; #49 encoding TBC
    wsPlan.Cells(planR, 34).value = wsLog.Cells(logR, 5).value ' AH KF
    wsPlan.Cells(planR, 35).value = wsLog.Cells(logR, 3).value ' AI Ry (Yaw)
    wsPlan.Cells(planR, 36).value = wsLog.Cells(logR, 4).value ' AJ Pitch
    wsPlan.Cells(planR, 37).value = wsLog.Cells(logR, 6).value ' AK DeltaYaw
    wsPlan.Cells(planR, 38).value = wsLog.Cells(logR, 7).value ' AL DeltaPitch
    wsPlan.Cells(planR, 39).value = wsLog.Cells(logR, 8).value ' AM Label
End Sub
