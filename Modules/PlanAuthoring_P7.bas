...
Public Sub AddPlanRowFromLog()
    ...
    ' fires at the astro event anchor, target=sun/moon/mw resolves to
    ...
    Dim defRy As Variant:        defRy = "—"     ' formula-fills if marker target
    Dim defRp As Variant:        defRp = "—"
    ...
    Select Case logType
        ...
        Case "astro"
            ...
            ' the astro object (target = sun/moon/mw) anchored on the astro
    ...
    Select Case anchorType
        ...
        Case "ASTRO"
            cellAnchorRef.Validation.Add Type:=xlValidateList, _
                AlertStyle:=xlValidAlertStop, _
                Operator:=xlBetween, _
                Formula1:="sunset,sunrise,moonrise,moonset,mwrise,mwtransit,mwset"
            cellAnchorRef.Validation.ShowInput = True
