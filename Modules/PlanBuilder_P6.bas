...
Private Sub WritePlanRow(ByVal wsPlan As Worksheet, _
    ...
    End If
    ' Col H = WP # — P6: text "WP01", "WP02", ... (was integer pre-P6).
    ' Anchor ref in middle zone uses the same convention so MATCH() in
    ' the Q (Fires at) anchor resolver formula is string-to-string.
    If Not IsEmpty(wpNum) Then
        wsPlan.Cells(r, 8).value = "WP" & Format(CLng(wpNum), "00")
    End If
