...
'
' Plan sheet right-zone columns (P6 layout):
'   AC=LogRow#, AD=Time, AE=Type, AF=AstroTgt, AG=KF,
'   AH=Ry,      AI=Pitch, AJ=Δyaw, AK=Δpitch,    AL=Label
' ============================================================
...
'   Plan right zone (P6: shifted +2):
'     AC=LogRow#, AD=Time, AE=Type, AF=AstroTgt, AG=KF,
'     AH=Ry,      AI=Pitch, AJ=Δyaw, AK=Δpitch,    AL=Label
'   Mapping: yaw -> AH, pitch -> AI, notes -> AL (label).
...
Private Sub WriteRowLegacy(ByVal wsLog As Worksheet, ByVal logR As Long, _
    ...
    wsPlan.Cells(planR, 29).value = logR                       ' AC — Log row#
    wsPlan.Cells(planR, 30).value = wsLog.Cells(logR, 1).value ' AD — Time
    ' AE Type left blank
    ' AF AstroTgt left blank
    ' AG KF left blank
    wsPlan.Cells(planR, 34).value = wsLog.Cells(logR, 2).value ' AH — Ry (from Yaw)
    wsPlan.Cells(planR, 35).value = wsLog.Cells(logR, 3).value ' AI — Pitch
    ' AJ Δyaw left blank
    ' AK Δpitch left blank
    wsPlan.Cells(planR, 38).value = wsLog.Cells(logR, 4).value ' AL — Label (from Notes)
End Sub
...
'   Plan right zone (P6): AC=LogRow#, AD=Time, AE=Type, AF=AstroTgt,
'                         AG=KF, AH=Ry, AI=Pitch, AJ=Δyaw, AK=Δpitch, AL=Label
'
'   Note on AF AstroTgt: The post-#49 firmware does NOT separately
...
'   elsewhere. For now we copy Type verbatim into AE and leave AF
...
Private Sub WriteRowRich(ByVal wsLog As Worksheet, ByVal logR As Long, _
                          ByVal wsPlan As Worksheet, ByVal planR As Long)
    wsPlan.Cells(planR, 29).value = logR                       ' AC LogRow#
    wsPlan.Cells(planR, 30).value = wsLog.Cells(logR, 1).value ' AD Time
    wsPlan.Cells(planR, 31).value = wsLog.Cells(logR, 2).value ' AE Type
    ' AF AstroTgt — leave blank for now; #49 encoding TBC
    wsPlan.Cells(planR, 33).value = wsLog.Cells(logR, 5).value ' AG KF
    wsPlan.Cells(planR, 34).value = wsLog.Cells(logR, 3).value ' AH Ry (Yaw)
    wsPlan.Cells(planR, 35).value = wsLog.Cells(logR, 4).value ' AI Pitch
    wsPlan.Cells(planR, 36).value = wsLog.Cells(logR, 6).value ' AJ DeltaYaw
    wsPlan.Cells(planR, 37).value = wsLog.Cells(logR, 7).value ' AK DeltaPitch
    wsPlan.Cells(planR, 38).value = wsLog.Cells(logR, 8).value ' AL Label
End Sub


' ============================================================
' Count existing right-zone rows (P6: col AD = Time)
' ============================================================
Private Function CountRightZoneRows(ByVal wsPlan As Worksheet) As Long
    ...
    For r = PLAN_FIRST_ROW To PLAN_FIRST_ROW + PLAN_MAX_ROWS - 1
        If Not IsEmpty(wsPlan.Cells(r, 30).value) Then n = n + 1
    Next r
