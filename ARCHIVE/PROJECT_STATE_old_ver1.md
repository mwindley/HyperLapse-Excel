...
**Last updated:** 21 May 2026 (Session C day 13 — #40 BNO085
integration architecture resolved across all six questions:
anchor mechanism = additive yaw offset on earth-frame gimbal
segments only, cart stays dumb on heading, no cart-path
correction. Pure design session, no code changes.)
...
`EXPOSURE_FALLBACK.md` — that contains the working agreement, the
gimbal visualisation design, the open task list, and the exposure
fallback design (with reference table as Appendix A).

---

## Day-13 session — #40 BNO085 integration architecture resolved

Pure design session. No code changes. All six architectural
questions raised in WORKFRONTS #40 are now resolved.
...
**Production fix applied and validated end-to-end (this session):**
...
**Workfronts resolved this session:**
- #1 (opto swap) — innocent, not needed for drops; spare 4N25s
  remain as inventory.
- #3 (post-opto Tv=0.8"+2s re-test, target 100%) — superseded.
  Tonight's Tv=0.5"/2s at 100% is a stricter condition.
- "Stage 4 milestone" open design decision — opto piece falls
  away; Stage 4 reduces to production-envelope soak only.
- "Logic-analyser-first vs opto-first" open design decision —
  moot. Analyser-first answered the question.
- #16 / #36c (time-based luminance fetch) — deleted, not just
  deferred. Resilience (skip-2-on-fail) works at any cadence; no
  delivery argument left after Day-12 fix; CPU argument too thin
  to justify touching working code.
- #9 (±180° yaw constants → cumulative) — done with the cleaner
  "cart dumb on yaw, Excel owns envelope" architecture rather
  than the original "widen constants" wording. Settings named
  ranges `gimbalYawEnvelopeMin` / `gimbalYawEnvelopeMax` (default
  ±225°, 450° span); GimbalPosition refuses out-of-envelope
  commands. SDK confirmed to honour cumulative yaw past ±180°.
- #36b (Formula evaluator on cart) — Excel pushes Appendix A
  parameters via GET query string (~1.3 KB URL, well inside the
  1.5 KB envelope verified via /debug/urlsize). Cart stores ~1.4
  KB RAM, walks parser/evaluator matching Excel's UDF logic.
  Verified end-to-end with 9 evaluation points + real Appendix A
  push (51/12/49/14 entries landed correctly). Diagnostic
  endpoint /debug/formula retained. Tv-format Canon translation
  ("0.5" → "0\"5") deferred to #36d which applies to camera.
- #36d subtask 1 (Time anchor on cart) — Excel sends both sunset
  and sunrise trel anchors plus the astronomical-sunset crossover
  threshold (`t0ss`, `t0sr`, `cross`). Cart advances both in
  lockstep from millis base, picks active event by sunset-trel
  vs cross. One push covers a full sunset-through-sunrise shoot.
  Verified end-to-end Day 12. `/debug/trel` reports full state.
  Remaining #36d subtasks: outage detection (threshold sizing),
  Tv-format translation, photo-loop integration, recovery
  smoothing, edge cases. Sketch now at 50% flash, 68% globals.
  Stale Settings cells + API HTTP 521 surfaced as operator-workflow
  concerns (refresh times before pushing) — noted, not blocking.
- #40 BNO085 first-light — Adafruit 4754 on Uno R4 over I2C,
  alive at 0x4A. Standalone `BNO085_BenchTest.ino` calibrates
  via figure-8 motion (acc=3 achievable), captures true-north
  offset against iPhone compass with single `c` command,
  tracks to within ±3° of iPhone across all four quadrants.
  Negligible error for 14mm lens (~3% of frame). NOT yet
  integrated into production sketch — architectural questions
  about continuous-vs-fixed cart heading, offset persistence,
  mid-shoot acc dropout, and frame-convention bridge raised in
  WORKFRONTS #40 for next session.

---

## Day-10 session — Smooth Selection built and rejected

Session focus: build and test the Smooth Selection workflow (#44
cluster) for cleaning wobbly recon CartLogs into executable plans.
Built end-to-end, proved working mathematically, then rejected on
operator-workflow grounds. Net outcome: simpler architecture and
clearer purpose for the trace chart.

### What was built

**`WobblyRecon.bas`** — `SimulateWobblyRecon` Sub writes a synthetic
16-row CartLog with 4 deliberately-wobbly clusters: fumbled left
bend (rows 5-8), wobbly straight (rows 8-11), clean right turn
(rows 11-13), wobbly finishing straight (rows 13-16). Test fixture
that exercises Smooth Selection without driving the cart.

**`BicycleModel.bas` extensions:**
- Trace sheet gained column H `CartLogRow` — populated only on the
  *final* sub-step of each T (steering) and X (stop) event,
  identifying which CartLog row each Trace point came from.
- `RefreshTraceChart` now uses Excel's linked-data-labels feature
  (`InsertChartField msoChartFieldRange`) — chart shows row-number
  labels at each T/X event point on the trace. Operator can match
  labels on the chart to rows in CartLog.
- `SecToHms` promoted from Private to Public so WobblyRecon and
  other modules can format synthetic timestamps consistently.

**`Smooth.bas`** (later rejected — see below):
- `btnSmoothSelection` reads operator's row selection (via
  `Worksheet_SelectionChange`-stashed `gLastCartLogSelection`),
  computes single-arc replacement using chord-to-arc geometry,
  writes proposal to hidden cells Q1:T(N), draws red overlay arc
  as second chart series, sets status banner.
- `btnCommitSmooth` rewrites CartLog rows in place, re-integrates.
- `btnDiscardSmooth` clears the proposal and overlay.
- `BuildCartLogButtons` paints three button cells (G1, H1, I1) +
  status banner (J1), hides storage columns Q:T.

**CartLog sheet code module:**
- `Worksheet_SelectionChange` stashes the operator's last data-row
  selection into `Smooth.gLastCartLogSelection` (cross-module
  Public variable). Survives the button double-click that would
  otherwise change live Selection out from under the macro.
- `Worksheet_BeforeDoubleClick` dispatches button-cell clicks to
  the three Smooth macros via `Buttons.RunButton`.

### Smooth verified working before rejection

Three test cases on the wobbly recon log:

- **Rows 5-8 (fumbled left bend):** R=9.3m, steer -3.0°, deviation
  544mm. Large deviation honestly identified — the signal that this
  cluster is too ambitious for a single arc. Operator could see by
  eye that the red overlay didn't perfectly hit the endpoint.
- **Rows 8-11 (wobbly straight):** R=97.9m near-straight, deviation
  ~80mm. Correctly identified the operator's intent as a straight
  segment. "A straight line is a series of curves" architectural
  principle validated — same maths handles both arcs and
  near-straights without a special case.
- **Commit:** CartLog reduced from 17 rows to 15. Re-integration
  drew a clean line in place of the S-shaped wobble.

### Why Smooth was rejected

Mathematical reason: a single arc has 1 free parameter (R) but
matching the end pose (Δx, Δy, Δθ) needs 3. The single-arc fit
matches chord + heading change, **not the end position exactly**.
The arc ends at a slightly different (x, y) than the original
wobbly drive did.

Operator-workflow reason: rows downstream of the smoothed segment
are integrated **from the new endpoint**, so the entire trace from
row j onwards shifts in (x, y). Operator's mental model "what I
selected is what I changed" breaks. They'd chase the drift by
smoothing the next segment, which would shift things again. <>
simple.

Architectural reason: no execution benefit. The cart drives by
distance + steering per segment. Whether the path is "wobbly" or
"smooth" doesn't affect cart-side execution. At 5 m/hr execution
speed, small wobbles are invisible in the captured photos. The
smoothing was cleaning up a chart appearance, not solving a real
execution problem.

Operator quote from session: *"i like the visualisation of the
log, the smooth is complex <> simple, i am tempted to reject smooth
requirement, the chart offers integration point for gimbal, much
better value in that complexity"*

### Architectural reframe (locked this session)

CartLog **is** the Plan. The recon drive at 100 m/hr produces the
executable plan directly. What the operator edits is:

1. **S-event speed values** — change the Value column from 100 m/hr
   (recon) to the desired execution speed per segment (5 m/hr
   photographable, 10 m/hr transitions, etc.). In-place cell edit,
   no maths.
2. **Nothing else.** The wobbles stay. The cart faithfully
   reproduces them at slow speed; they cost nothing in the photos.

The chart's role is **gimbal-plan integration**: with row-number
labels at each T/X event, the operator references cart waypoints
(W5, W12, etc.) when authoring gimbal-plan rows.

### Why 100 m/hr recon → 5 m/hr execution makes sense

Operator framing this session: at 100 m/hr the cart moves ~28 mm/s,
fast enough for the operator to steer reactively with normal
hand-eye coordination. At 5 m/hr it moves ~1.4 mm/s; a steering
input takes 30+ seconds to show visible effect. Open-loop driving
with delayed feedback. Operator quote: *"like driving car with eyes
closed for 30 sec then open for 5 sec"*.

So: operator at 100 m/hr is the controller; the captured CartLog
becomes the plan; cart at 5 m/hr autonomously executes the plan
while operator supervises (not steers). The two-speed split is what
the BicycleModel + plan endpoints architecture exists to serve.

### Build lessons learned this session

1. **VBA `Application.Run` requires module-qualified macro names
   across modules.** `Application.Run "btnSmoothSelection"` fails
   silently with "Cannot run the macro" even though
   `btnSmoothSelection` is Public in module `Smooth`. Fix:
   `Application.Run "Smooth.btnSmoothSelection"`. Bare names work
   only within the same module, or in some special cases
   (`btnIntegrateBicycle` worked bare from the Control sheet
   handler — likely a project-level namespace search). Failure
   mode is silent: RunButton catches the error, paints yellow,
   logs "Cannot run the macro" — no MsgBox.

2. **`SelectionChange` handler is the right way to remember
   operator's selection across button clicks.** A button-cell
   double-click changes live `Selection` out from under the
   triggered macro. Solution: in the sheet's code module, write
   `Worksheet_SelectionChange` that stashes data-row selections
   into a module-level variable. The macro reads from the stash.

3. **Cross-module Public variables are simpler than custom sheet
   properties.** Attempted `wsLog.LastDataSelection` (sheet code-
   module Property Get) failed at compile time — generic
   `Worksheet` type doesn't resolve custom sheet members. A Public
   variable in a regular `.bas` module is cleaner. Naming
   convention `gLastSomething` for module-level Public state.

4. **Excel data labels: use `InsertChartField msoChartFieldRange`
   to link labels to a cell range.** `.DataLabels(i).Text = ""`
   does NOT suppress the label — Excel falls back to showing the
   value. The correct pattern is `.HasDataLabels = True`,
   `.DataLabels.ShowValue = False`, `.DataLabels.ShowRange = True`,
   then `.Format.TextFrame2.TextRange.InsertChartField
   msoChartFieldRange, "='Sheet'!$H$2:$H$N", 0`. Blank cells in
   the range produce blank labels.

5. **Excel auto-converts time strings on write.** Writing
   `"00:00:05"` to a Number-formatted cell stores `5.787E-05` (a
   day fraction). Set `Columns(1).NumberFormat = "@"` **before**
   writing rows, not after.

### What to remove from the workbook before next session

- `Smooth.bas` module — entire file
- CartLog sheet code module — remove the two handlers (or leave;
  harmless without the buttons)
- Button cells G1:I1 on CartLog (Smooth Selection / Commit Smooth /
  Discard Smooth)
- Named ranges `btnSmoothSelection`, `btnCommitSmooth`,
  `btnDiscardSmooth`
- Hidden columns Q:T on CartLog (proposal storage) — unhide and
  clear

What to **keep** from this session's build:
- `WobblyRecon.bas` (synthetic log generator — useful test fixture)
- `BicycleModel.bas` extension (row labels on chart, `CartLogRow`
  column, public `SecToHms`)

### Files added/modified this session

- `WobblyRecon.bas` (NEW, ~220 lines)
- `BicycleModel.bas` (MODIFIED — Trace col H, chart labels via
  InsertChartField, `SecToHms` Public)
- `Smooth.bas` (NEW ~700 lines — TO BE REMOVED)
- CartLog sheet code module (MODIFIED — to be reverted)

### What's next

Pending Excel cluster work (suggested order):
1. **#45 Speed editing in CartLog** — small, validates the
   "CartLog is the Plan" model end-to-end. Check whether today's
   `/plan/load` segment format honours per-segment SPEED values.
2. **#14a Astro endpoint computation** — small, isolated, fills
   gimbal-row computed columns
3. **#14 Catmull-Rom evaluator + #15 viz chart** — gimbal plan
   authoring; chart on CartLog (with row labels) is the upstream
   input
4. **#46 Gimbal authoring against cart row labels** — design pass
   redefining the GimbalPlan schema (no separate CartPlan sheet)

PREFERENCES candidate to add next time it's edited:
*"Visualisation > Manipulation: a clear visualisation of what the
operator did is more valuable than a tool to mathematically clean
it up. Operator's eye + redrive is simpler than algorithmic
smoothing, and avoids drift / knock-on / trust-breach problems
from edit-after-the-fact on integrated state."*
Worked example: #44 Smooth, built and rejected day 10.

---

## Day-9 late evening session — simple fallback formula built

Session focus: build the Excel-side simple formula that approximates
the Old World Table, ready to push to cart at shoot time. CCAPI
fallback path (#36 + #36a) now end-to-end on the Excel side.

### What was built

**New module `Formula.bas`** in the workbook, containing:
- `InitFallbackFormula` — one-shot setup, builds the new
  FallbackFormula sheet and seeds default column from
  Appendix A.
- `FormulaTv(t_rel, branch)` — Excel UDF
- `FormulaISO(t_rel, branch)` — Excel UDF
- `PushFormulaToCart` — POSTs active branch's parameters to
  cart `/exposure/load`.
- `AddBranch newName, copyFromName` — clones a branch column.

**New sheet `FallbackFormula`** with default column holding:
- 51 sunset Tv crossover t_rel values (from Appendix A)
- 12 sunset ISO ramp anchors
- 3 policy ceilings (Tv ceiling 20s, ISO ceiling 1600, ISO base 100)
- Live evaluator (rows 78-82) — operator scrubs t_rel cell,
  watches Tv and ISO update

**New named range `dataActiveBranch`** on Settings sheet
($C$44, label at $B$44). Set to "default" initially.

**Two new buttons** on Control sheet (via updated `Buttons.bas`):
- "Init Fallback Formula" → `InitFallbackFormula`
- "Push Formula to Cart" → `PushFormulaToCart`

Handler updated on Control sheet's `Worksheet_BeforeDoubleClick`
to wire the two new buttons through `RunButton`.

### Verified

Formula reproduces Appendix A at seven test points spanning all
phases (deep daylight, pre-sunset, fast twilight, slow Tv, ISO-base
ceiling, mid ISO ramp, ISO ceiling). All matched exactly.

Cart push tested with cart powered off — got HTTP 0 (no server) as
expected. Excel side built 937-byte JSON payload for default branch
and made the POST. LogEvent captured the attempt. Re-test against
running cart firmware will yield HTTP 404 until firmware #36b
implements the `/exposure/load` endpoint.

### Build lessons (worth adding to PREFERENCES next session)

Three VBA / Excel gotchas surfaced during the build:

1. **VBA line continuations capped at ~24 per logical line.** Long
   `Array(Array(...), Array(...), ...)` literals fail to compile.
   Use row-by-row assignment to a pre-sized array instead.

2. **Excel parses cell strings starting with `==` as formulas.**
   Section headers like `"== Sunset Tv crossovers =="` raise
   runtime error 1004 (`Application-defined or object-defined`)
   on write. Use `--` or prefix with apostrophe. The error message
   doesn't hint at this; debugging required ~10 minutes of
   process-of-elimination including Immediate Window writes that
   succeeded on short strings and failed on the leading-`==` form.

3. **`With Range.Cells(r,c)` blocks** can fail with 1004 after
   recent operations on adjacent cells, even when direct
   `range.Cells(r,c).property = ...` works on the same cell. The
   `With` syntax was abandoned in favour of repeated direct cell
   access during this session — and turned out to be unrelated to
   the actual `==` bug, but the rewrite stayed because direct
   access is more robust.

### Sunrise side note

Formula currently stores sunset side only. Sunrise = time-mirror
of sunset is the design assumption (EXPOSURE_FALLBACK.md §3.2).
Sign-flip in the UDF or a second column block is a small later
enhancement. Not blocking for first CCAPI shoot since shoots
typically start near sunset.

### Files added/modified this session

- `Modules/Formula.bas` (NEW, ~600 lines)
- `Modules/Buttons.bas` (UPDATED, added 2 button entries)
- Control sheet `Worksheet_BeforeDoubleClick` handler
  (manual edit, +4 lines)
- New sheet `FallbackFormula` in workbook
- New named range `dataActiveBranch` on Settings sheet
- `WORKFRONTS.md` (#36 + #36a marked DONE)
- This file (PROJECT_STATE.md)

### What's next

Still all the cart firmware work (#36b, #36c, #36d, #36e) before
CCAPI fallback is end-to-end. None blocking the immediate next
shoot — operator can pick branch and push to cart now; cart
firmware will adopt the formula when it's ready.

Post-timelapse session workflow (#37 cluster) not yet built.
Today's verification confirms the cart push pipeline is in place;
the post-timelapse import workflow is the next Excel piece.

---

## Day-9 evening session — exposure validation + UI re-baseline

Session focus: exposure recipe validation against real-world shoots,
UI polling re-baseline tests with day-9 avoidance code in place.

### Exposure recipe validation (workfront #36)

Established role: hand-built sun table is **fallback** when CCAPI/WiFi
out; primary path is live luminance reading. Cart switches between
paths as availability changes. Full design in `EXPOSURE_FALLBACK.md`.

Goal of collecting real-world exposure exports: refine the fallback
table over time, not replace with formula.

Analysed two shoots:
- **April 17-18 2026 (DST-corrected):** Sunset side 73% within ±0.5
  stop, 95% within ±1 stop. Sunrise side incomplete (operator
  skipped ISO ramp).
- **Feb 20-21 2026:** Sunset/sunrise mid-curve 82-84% within ±1
  stop. Astro-lock zone showed -2.5 stop offset — **likely manual
  time-nudge artefact** per operator (old Excel had nudge feature,
  believed used on this shoot).

Three signals identified:
1. Post-sunset rapid twilight: old recipe ~0.5 stop too dark
   (consistent across shoots) — refinement candidate.
2. Astro-lock boundaries: large offsets, but likely operator-nudge
   artefact, not table error.
3. Mid-curve fast-Tv: solid.

### Method gotchas captured (now in EXPOSURE_FALLBACK.md §5.6)

- **Excel date-mangling of Tv column:** fractional Tv values parsed
  as dates by Excel. Full decoder rules documented.
- **Camera-clock DST:** April shoot timestamps were 1 hour off
  (camera on ACDT past DST end). Verify via Tv inflection points.
- **Manual time-nudge:** operator can shift entire ramp ±10-15 min
  via old Excel feature, not visible in EXIF. Flag shoots showing
  large astro-lock offsets as suspect.

### Sky-condition axis (new design point)

Refined table likely needs 2-3 variants (clear/partial/overcast),
operator-tagged at shoot start. Weather not in EXIF — needs operator
log or post-hoc weather-API lookup. See EXPOSURE_FALLBACK.md §5.3.

### UI polling re-baseline tests (workfront #27)

Both tests at Tv=0.8" + 3s interval, darken mode, fetchdelay=0,
15-min runs, day-9 avoidance code active.

| Test | UI tabs | Fires | On card | Delivery |
|---|---|---|---|---|
| A | Closed | 265 | 256 | **96.6%** |
| B | Open, polling at 3s | 309 | 303 | **98.1%** |

(Prior baseline pre-day-9: Tv=0.8" + 2s + UI open + 1s polling +
no favicon fix = 76%)

**Findings:**
- Cart side rock-solid: pin-8 pulses pristine (rise/fall 6-7µs,
  high 28395-28405µs), gap always 3000-3010ms.
- All drops are camera-side (pin-8 fired but no red LED).
- UI-open with day-9 avoidance ≈ UI-closed. Polling no longer
  saturates.
- Original 76% drop **not predominantly UI-polling fault**.
  Interval (2s vs 3s) and/or camera-busy time at Tv=0.8" are
  larger contributors. Residual 2-4% drop is camera/opto-side.
- Logic analyser test plan when it arrives: opto both-sides
  measurement at Tv=0.8" production edge.

### Workfront #27 status

Resolved-via-avoidance verified empirically. Polling no longer a
delivery risk in current code. **Not closed** — production shoots
may re-expose at longer durations.

### Tests NOT completed this evening

- Tv=1/5000 @ 2s interval (no-camera-stress control) — started,
  halted due to faults at the moment, not retried.

### Files modified this evening

- None on cart firmware.
- Workfronts #36-39 added to WORKFRONTS.md (exposure fallback +
  validation cluster).
- Workfronts #40-42 added to WORKFRONTS.md (heading + gimbal
  stream cluster, absorbing prior compass-anchor and gimbal-rate
  draft notes).
- `EXPOSURE_FALLBACK.md` created — single canonical exposure
  design doc, absorbs prior `CCAPI_FALLBACK.md`, `WORKFRONT_36.md`,
  `OLD_SUN_TABLE.md`.
- `SHOPPING.md` updated with BNO085 line item and refined mast
  spec; `SHOPPING_ADDITIONS.md` retired.

### Next session candidates

1. **#38 design** — Excel workflow for importing new exports and
   managing the dataset, including sky-condition tagging.
2. **#28 front-vs-rear step investigation** — still open from day 9
   morning; should happen before #29 circle test.
3. **#29 circle/symmetry/linearity** for SERVO_TO_DEG refinement.
4. **#32 't' event integration** in BicycleModel.bas.
5. **Logic analyser test plan** for when TOL-18627 arrives — opto
   both-sides measurement at Tv=0.8" production edge.
6. **Tv=1/5000 @ 2s control test** — finish what was halted today.
